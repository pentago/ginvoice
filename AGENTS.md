...
- `internal/pdf/` — HTML/CSS → PDF via go-weasyprint/webrender (pure Go, `CGO_ENABLED=0`, no browser). Layout lives in `internal/pdf/invoice.html` (a `text/template`; `render.go` pre-escapes all user data). Users can override it at `/data/invoice_template.html` — invalid templates are logged and ignored (built-in wins). Fonts: static DejaVu TTFs embedded from `internal/pdf/fonts/`, extracted to `/data/fonts` on first render (the fontconfig port resolves fonts by file path; `/data` overridable via `GINVOICE_DATA_DIR`). Gotchas: must use the Pango font configuration (the gotext engine's text drawing is unimplemented upstream); variable fonts and TTC collections silently produce invisible text.
...
- **Static binary**: `CGO_ENABLED=0` always (modernc.org/sqlite and the PDF stack are pure Go). Docker runtime is `distroless/static`.
...
- **Logo storage**: logo is stored as a base64 data URI string in the `logo_data` column of `companies`, not on disk. The PDF template references the data URI directly in `<img src>`; go-weasyprint's default URL fetcher resolves `data:` URIs itself.
